...
function botMove(hand: any, player: any): BotDecision {
  normalizeHand(hand);
  const playerBet = hand.roundBets?.[player.id] ?? 0;
  ...
  if (callAmount <= 0) {
    if (premiumPreflop) return { move: 'bet', amount: potBetAmount(hand, player, 0.5) };
    if (hand.stage !== 'preflop' && scoopScore >= 5) return { move: 'bet', amount: potBetAmount(hand, player, 0.75) };
    if (hand.stage !== 'preflop' && scoopScore >= 3) return { move: 'bet', amount: potBetAmount(hand, player, 0.25) };
    return { move: 'check' };
  }
