...
const clientGeneratedOutput = new URL('../../demo/client/src/generated/', import.meta.url);
const serverGeneratedOutput = new URL('../../server/src/generated/', import.meta.url);
...
const illustratedCitySet = new Set(illustratedCities);
...
const cities = await Promise.all(dataset.cards.map(async card => {
  ...
  const landmarkImage = illustratedCitySet.has(card.slug)
    ? await readFile(new URL(`./landmarks/${card.slug}.png`, import.meta.url))
    : null;
  return {
    ...card,
    ...(visualOverrides[card.slug] ?? {}),
    flagDataUri: `data:image/svg+xml;base64,${Buffer.from(flagSvg).toString('base64')}`,
    landmarkImageDataUri: landmarkImage ? `data:image/png;base64,${landmarkImage.toString('base64')}` : null,
...
await mkdir(clientGeneratedOutput, { recursive: true });
await mkdir(serverGeneratedOutput, { recursive: true });
...
const manifest = JSON.stringify({ cities: illustratedCities }, null, 2);
await Promise.all([
  writeFile(new URL('manifest.json', output), manifest, 'utf8'),
  writeFile(new URL('manifest.json', publicOutput), manifest, 'utf8'),
  writeFile(
    new URL('illustratedCities.ts', clientGeneratedOutput),
    `// Generated by tools/city-cards/generate.mjs.\nexport const ILLUSTRATED_CITY_SLUGS = ${JSON.stringify(illustratedCities, null, 2)} as const;\n`,
    'utf8',
  ),
  writeFile(
    new URL('illustratedCapitals.ts', serverGeneratedOutput),
    `// Generated by tools/city-cards/generate.mjs.\nexport const ILLUSTRATED_CAPITAL_SLUGS = ${JSON.stringify(illustratedCities, null, 2)} as const;\n`,
    'utf8',
  ),
]);
