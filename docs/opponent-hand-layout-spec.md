...
the card face and adds the high/low combination block below the cards. It must
not move or resize the hand zone, the name/action row, or the card row.
...
An active opponent is indicated by a yellow outline around the name badge.
There is no `THINKING` text in the opponent hand zone. The indicator must not
change the card coordinates, frame size, or slot size.
